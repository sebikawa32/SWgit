...
function Header({ isLoggedIn: externalIsLoggedIn }) {
  ...
  const fetchPopularKeywords = async () => {
    try {
      const res = await axios.get("/api/keywords/popular");
      ...
      <nav className="navbar">
        ...
        <div className="nav-right">
          <div className="search-container" style={{ position: "relative" }}>
            ...
            {shouldShowDropdown && (
              <ul className="search-dropdown" ref={dropdownRef} onMouseEnter={handleDropdownMouseEnter} onMouseLeave={handleDropdownMouseLeave}>
                {popularKeywords.map((item, idx) => (
                  <li key={idx} onClick={() => handleKeywordClick(item.keyword)}>
                    <span style={{ fontWeight: 600, color: "#c0c0c0" }}>{idx + 1}위</span>&nbsp;{item.keyword}
                  </li>
                ))}
              </ul>
            )}
          ...
          {isLoggedIn ? (
            <>
              ...
              <div className="nickname-wrapper" ref={myMenuRef} onMouseEnter={handleMyMenuMouseEnter} onMouseLeave={handleMyMenuMouseLeave}>
